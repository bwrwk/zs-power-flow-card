...
export class ZsPowerFlowCard extends LitElement {
        ...
        <section class="shell">
          ...
          <div class=${`stage ${layoutClass} ${presetClass} ${flowStyleClass} ${advanced ? 'stage-advanced' : 'stage-simple'}`}>
            ...
            ${this._config.show_solar
              ? this.renderNode(
                  snapshot.solar,
                  this.getNodePosition('solar'),
                  'solar',
                  this._config.solar_entity,
                  undefined,
                  this.getConsoleNodeMetrics(snapshot, 'solar'),
                )
              : nothing}
            ${this._config.show_grid
              ? this.renderNode(
                  snapshot.grid,
                  this.getNodePosition('grid'),
                  'grid',
                  this._config.grid_entity,
                  undefined,
                  this.getConsoleNodeMetrics(snapshot, 'grid'),
                )
              : nothing}
            ...
            ${this.renderNode(
              snapshot.home,
              this.getNodePosition('home'),
              'home',
              this._config.home_entity,
              undefined,
              this.getConsoleNodeMetrics(snapshot, 'home'),
            )}
  ...
  private getNodePosition(iconName: 'solar' | 'grid' | 'battery' | 'home') {
  ...
  private getConsoleNodeMetrics(
    ...
    if (this._config.visual_preset !== 'console') return [];
  ...
  private buildConsoleAnchoredPath(
  ...
  private buildRoundedPolylinePath(points: Array<{ x: number; y: number }>, radius: number) {
